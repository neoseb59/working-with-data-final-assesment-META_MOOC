...
export function getSectionListData(data) {
  ...
  const groupByCategory = data.reduce((groupByMap, menuItem) => {
    const group = (groupByMap[menuItem.category] || []);
    group.push(menuItem);
    groupByMap[menuItem.category] = group;
    return groupByMap;
  }, {});

  const createObjectFromGrouping = ([category, menuItemList]) => {
    return {
      title: category,
